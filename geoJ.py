...
class GeoJ:
    ...
    def __init__(self, geoJFile, isString=False):
        if isString:
            try:
                self.geoJFile = geoJFile
            except IOError:
                print(
                    "Error: can not find file. Make sure the file name and path are correct"
                )
            else:
                try:
                    self.geoJObj = json.loads(self.geoJFile)
                except ValueError:
    ...
    def toShp(self, shpFile):
        ...
        if self.geometryType == 'Point':
            ...
            print('Can not proceed. The geometry type ' + self.geometryType +
